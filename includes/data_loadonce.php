...
$_ginfo["query"]["showaccountbalance"] = "select uid, sum(amount+PayUMoney) as mymoney from moneyaccount group by uid";
...
//$_ginfo["query"]["lastcalldetail"] = "select caller_call.st_id, caller_call.teacher_id,caller_call.comments,caller_call.created_at as caller_date ,caller_demo.name as demo ,caller_demo.id as demo_id FROM caller_call INNER JOIN (SELECT MAX(created_at) as lasttime FROM `caller_call` GROUP BY st_id) maxval ON caller_call.created_at=maxval.lasttime AND caller_call.demo_id != '7' LEFT JOIN caller_demo ON caller_call.demo_id = caller_demo.id";
//$_ginfo["query"]["caller_st_number"] = "select count(st_id) as caller_st_num,teacher_id from ".qtable('lastcalldetail')." where lastcalldetail.demo_id!=7  group by teacher_id";
