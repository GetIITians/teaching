        ...
        <div class="card teacherlistelm">
			...
			*/ ?>
            ...
            <?php endif; ?>
                ...
                <p class="grey-text text-darken-2">
                    Subjects
                    <a class="tooltipped" data-placement="right" data-toggle="tooltip"
                       title="<?php echo $subject_list; ?>">
                        <i class="tiny material-icons">label</i>
                    </a>
                </p>
                ...
                <p class="grey-text text-darken-1">Experience
                    : <?php echo(($row['teachingexp'] == 0) || ($row['teachingexp'] == 1) ? (($row['teachingexp'] == 0) ? "None" : $row['teachingexp'] . ' Year') : $row['teachingexp'] . ' Years'); ?> </p>

                <p class="grey-text text-darken-2">
                    Languages
                    <a class="tooltipped" data-placement="right" data-toggle="tooltip"
                       title="<?php echo convert_lang($row['lang']); ?>">
                        <i class="tiny material-icons">label</i>
                    </a>
                </p>
                <?php if ($age > 0) : ?>
                ...
                <p class="lastp">
                    <?php if (!$row["isdonedemo"]) : ?>
                        <a href="<?php pit(BASE . "profile/" . $row["tid"] . "/" . "5", User::islogin(), BASE . "login"); ?>">
                            <button type="button" class="btn waves-effect waves-light btn-small">1 Hour Trial Class
                            </button>
                        </a>
                    <?php endif; ?>
                    <!--
                ...
                <div class="rating" data-tid="<?php echo convchars($row['tid']); ?>">
                    ...
                    <?php if ($user = User::loginId()) : ?>
                        ...
                        ?>
                        <div class="rating-system" data-uid="<?php echo convchars($user); ?>"
                             previousRating="<?php echo $prev_rating; ?>" ratingId="<?php echo $rating_id; ?>">
                            <?php
                            if ($prev_rating !== 0) {
                                for ($i = 1; $i < 6; $i++) {
                                    if ($i <= $prev_rating)
                    ...
                    <?php endif; ?>
                    <div class="rating-value" count="<?php echo $row['rating_total']; ?>"
                         value="<?php echo round($row['rating'], 2); ?>">
                        <?php if ($row['rating_total'] > 0) : ?>
                            <p>
